...
Mac is a best-effort local runtime. For reliable native video QA, the same repo can run on a private EC2 GPU host. The planned client demo is screen-share based, so the EC2 app should bind to `127.0.0.1` and be accessed with SSH port forwarding rather than a public URL.
